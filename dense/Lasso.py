...
tr_set = MyDataSet(x_path='../dataset/bgedv2_X_tr_float64.npy', y_path='../dataset/bgedv2_Y_tr_'+dataset+'_float64.npy')
...
Y_va = torch.from_numpy(np.array(np.load('../dataset/bgedv2_Y_va_'+dataset+'_float64.npy'))).type(torch.FloatTensor).cuda()

X_te = torch.from_numpy(np.array(np.load('../dataset/bgedv2_X_te_float64.npy'))).type(torch.FloatTensor).cuda()
Y_te = torch.from_numpy(np.array(np.load('../dataset/bgedv2_Y_te_'+dataset+'_float64.npy'))).type(torch.FloatTensor).cuda()
...
outlog = open('../res/dense/Lasso-'+str(lamda_l1)+'-'+dataset+'.log', 'w')
...
for epoch in range(n_epoch):
    for i, data in enumerate(tr_loader, 0):
        ...
        regression_loss = criterion(tr_outputs, y_batch)

        loss = regression_loss + lamda_l1 * regularization_loss
        # backward
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
torch.save(net_parameters, '../res/dense/Lasso-'+str(lamda_l1)+'-'+dataset+'.pt')
